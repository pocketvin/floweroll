...
- Planner 传输重试已收敛到 Runtime 一层；恢复请求走 recovery_context，不额外叠加 Adapter 重试。新节点记录在开发观察台现有 Metrics 中，无独立 Graph UI。
...
- Planner 使用 OpenAI-compatible adapter；具体 Provider 由运行配置决定。
