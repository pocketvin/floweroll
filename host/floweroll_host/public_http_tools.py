...
class PublicHTTPToolSet:
    ...
    def __init__(self, *, resolver=None, opener=None) -> None:
    ...
    def fetch(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(url, str) or not url.strip():
            raise FunctionToolError("url must be a non-empty string", error_kind="model_correctable")
        normalized = self._validate_public_https(url.strip())
        ...
        try:
            ...
            raise FunctionToolError(f"network error: {exc.reason}", error_kind="transient") from exc
        except TimeoutError as exc:
            raise FunctionToolError("HTTP request timed out", error_kind="transient") from exc
    ...
    def _validate_public_https(self, url: str) -> str:
        ...
        if parsed.username is not None or parsed.password is not None:
            raise FunctionToolError("credentials in URLs are not allowed", error_kind="model_correctable")
        port = parsed.port or 443
        try:
            answers = self._resolver(parsed.hostname, port, type=socket.SOCK_STREAM)
        except OSError as exc:
            raise FunctionToolError("could not resolve URL host", error_kind="transient") from exc
        addresses = {answer[4][0] for answer in answers if answer and len(answer) >= 5}
        if not addresses:
            raise FunctionToolError("URL host resolved to no address", error_kind="transient")
...
def register_public_http_capabilities(
    ...
    adapter = FunctionToolAdapter(capability_id=spec.name, source_kind="http_api", read_only=True)
    registry.register(
...
def _bounded_json(value: Any, *, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        return {str(k): _bounded_json(v, depth=depth + 1) for k, v in list(value.items())[:50]}
